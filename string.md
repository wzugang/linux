...
${string#substring}                 从变量$string的开头, 删除最短匹配$substring的子串（可以与*通配符配合使用）
${string##substring}                从变量$string的开头, 删除最长匹配$substring的子串（可以与*通配符配合使用）
${string%substring}                 从变量$string的结尾, 删除最短匹配$substring的子串（可以与*通配符配合使用）
${string%%substring}                从变量$string的结尾, 删除最长匹配$substring的子串（可以与*通配符配合使用）
...
echo ${str%a*c}     # abbc,def,ghi,abcjkl    
     注意：必须从字符串的第一个字符开始，或者从最后一个开始，可以这样记忆,井号（#）通常用于表示一个数字，它是放在前面的；百分号（%）卸载数字的后面; 或者这样记忆，在键盘布局中，井号(#)总是位于百分号（%）的左边(即前面) 。
...
更多资料请参考：
1.字符串截取方法：http://www.cnblogs.com/xwdreamer/p/3823463.html
2.字符串处理：http://www.cnblogs.com/fengbohello/p/5954895.html
